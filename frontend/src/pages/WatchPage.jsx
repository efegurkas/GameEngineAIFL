...
export default function WatchPage() {
  ...
  // Board wrapper ref — log paneli bu elementin yüksekliğini ölçerek eşitler
  const boardColRef = useRef(null)
  const [boardColHeight, setBoardColHeight] = useState(null)
  ...
  // Board sütununun gerçek yüksekliğini ölç → log paneli bu değeri kullanır
  useEffect(() => {
    if (!boardColRef.current) return
    const obs = new ResizeObserver(([entry]) => {
      setBoardColHeight(entry.contentRect.height)
    })
    obs.observe(boardColRef.current)
  ...
  return (
    ...
    <div style={{ height: '100vh', width: '100vw', display: 'flex', flexDirection: 'column', background: '#050709', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        width: '100%',
        position: 'relative',
        // İçerik alanı navbar'ın altında kalır, log paneli bu context içinde konumlanır
        zIndex: 1,
      }}>
        ...
        {error ? (
          ...
          <div style={{ padding: '24px 20px 60px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <div style={{
              ...
              {/* Sol: Tahta Sütunu — yüksekliği ResizeObserver ile ölçülür */}
              <div
                ref={boardColRef}
              ...
              {/* Sağ: Log Sütunu
                  - position: sticky + top: 0 → scroll edilince board ile aynı hizada sabit kalır
                  - zIndex: 10 → diğer içeriklerin üstünde görünür
                  - height: boardColHeight → board sütunuyla birebir eşleşir, null iken fallback 500px
              */}
              <div style={{
                width: 320,
                height: boardColHeight ?? 500,
                display: 'flex',
